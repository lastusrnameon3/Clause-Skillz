...
| C — Corporate Windows machine | **Done, cause unexplained** | Follow README → *Work machine (Windows, managed settings)*: clone, set excludes, copy rules + skills only. First check: does `/context` show the 5 universal rules? Hooks won't run there: managed settings block user-level hooks, so `credential-guard` is advisory only at work. Decided 2026-09-24: no workaround. **2026-09-25:** skills first landed but didn't load; later the same day all loaded in TUI and VS Code with no deliberate change — cause unknown. Managed settings on that machine include `allowManagedPluginsOnly`, `disableSkillShellExecution`, `disableCustomSkillsCode` (the first and last aren't in the public settings reference). If skills stop loading, suspect those first; the approved route is an IT-managed plugin. No workaround that relabels skills as rules. |
...
- `03-writing` session-summary format: **keep for now (2026-09-25)** — it's the fallback if managed settings start blocking skills at work. Revisit the Audit A1 M2 split once skills at work have been stable for a few weeks.
